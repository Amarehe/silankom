...
<head>
    <meta charset="utf-8">
    <title>Tanda Terima Pengembalian - {{ $nomorSuratPengembalian }}</title>
    <style>
        @page {
...
<body>
    ...
    <!-- JUDUL -->
    <div class="judul">
        <h2>TANDA TERIMA PENGEMBALIAN PERALATAN ELEKTRONIK</h2>
    </div>

    <!-- NOMOR SURAT -->
    <div class="nomor-surat">
        Nomor: {{ $nomorSuratPengembalian }}
    </div>

    <!-- ISI -->
    <div class="isi">
        Penyerahan Pengembalian Peralatan Lembaga yang dikembalikan oleh :
    </div>
    ...
    <table class="data-tabel">
        ...
        <tr>
            ...
            <td>:</td>
            <td>{{ ucfirst($peminjaman->kondisi_kembali ?? '-') }}</td>
        </tr>
    </table>

    <!-- LOKASI DAN TANGGAL -->
    <div class="lokasi-tanggal">
        Jakarta, {{ \Carbon\Carbon::parse($peminjaman->tanggal_kembali)->locale('id')->isoFormat('D MMMM Y') }}
    </div>
    ...
    <div class="ttd-container clearfix">
        ...
        <div class="ttd-box right">
            ...
            <div class="jabatan-detail">
                {{ $adminPenerima->jabatan->nm_jabatan ?? 'Kepala Bagian' }}<br>
                {{ $adminPenerima->unitkerja->nm_unitkerja ?? 'Bag Komlek' }}
            </div>
            <div class="nama-penandatangan">{{ $adminPenerima->name ?? '-' }}</div>
            <div class="nip-penandatangan">NIP. {{ $adminPenerima->nip ?? '-' }}</div>
        </div>
    </div>
</body>
